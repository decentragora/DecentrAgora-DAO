...
export async function deployDAgora (): Promise<dAgora> {
    const DAgoraMock: ContractFactory = await ethers.getContractFactory('dAgora');
    const dAgora: dAgora = (await dAgora.deploy()) as dAgora;
