...
import { DAgoraMock, Governance } from '../typechain';

describe('Governance', function () {

    let governance: Governance, dAgora: DAgoraMock;
    ...
    before(async function () {
        await setupSigners();
        dAgora = await helpers.deployDAgora();
        governance = await helpers.deployGovernance();
        await governance.initialize(dAgora.address);
    ...
    describe('activate', function () {
        ...
        it('activates if threshold is met', async function () {
            await dAgora.setBondStaked(BigNumber.from(400000).mul(helpers.tenPow18));
    ...
    describe('propose', function () {
        before(async function () {
            await dAgora.setBondStaked(BigNumber.from(400000).mul(helpers.tenPow18));
            await governance.activate();
            await dAgora.setBondStaked(0);
        });

        it('create new proposal revert reasons', async function () {
            await dAgora.setBondStaked(amount);
            ...
            await dAgora.setVotingPower(userAddress, amount.div(10));
            await expect(governance.connect(user)
        ...
        it('cast, cancel and change vote', async function () {
            await dAgora.setBondStaked(amount);
            await dAgora.setVotingPower(userAddress, amount.div(10));
            await dAgora.setVotingPower(await voter1.getAddress(), amount.div(10));
            await dAgora.setVotingPower(await voter2.getAddress(), amount.div(10));
            await dAgora.setVotingPower(await voter3.getAddress(), amount.div(10));
        ...
        it('verify proposal state', async function () {
            ...
            expect(await dAgora.withdrawHasBeenCalled()).to.be.true;
        });
        ...
        it('fail for invalid quorum', async function () {
            await dAgora.setBondStaked(amount);
            await dAgora.setVotingPower(userAddress, amount.div(2));
        ...
        it('fail for invalid minimum threshold', async function () {
            await dAgora.setBondStaked(amount);
            await dAgora.setVotingPower(userAddress, amount.div(2));
            await dAgora.setVotingPower(await voter1.getAddress(), amount.div(2));
        ...
        it('allows anyone to cancel a proposal if creator balance fell below threshold', async function () {
            ...
            await dAgora.setVotingPower(userAddress, 0);
    ...
    describe('abrogation proposal', function () {
        ...
        it('fails if user does not voting power above threshold', async function () {
            ...
            await dAgora.setVotingPower(await somebody.getAddress(), amount);
        ...
        describe('voting', function () {
            ...
            it('reverts if user does not have voting power', async function () {
                ...
                await dAgora.setVotingPower(await voter3.getAddress(), 0);
    ...
    async function setupEnv () {
        await dAgora.setBondStaked(amount);
        await dAgora.setVotingPower(userAddress, amount.div(5));
        await dAgora.setVotingPower(await voter1.getAddress(), amount.div(20));
        await dAgora.setVotingPower(await voter2.getAddress(), amount.div(5));
        await dAgora.setVotingPower(await voter3.getAddress(), amount.div(5));
    }

    async function createTestProposal () {
        const targets = [DAgora.address];
        const values = [0];
