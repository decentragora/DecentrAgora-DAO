...
contract Governance is Bridge {
    ...
    // executed only once
    function initialize(address dAgoraAddr) public {
        require(isInitialized == false, "Contract already initialized.");
        require(dAgoraAddr != address(0), "dAgora must not be 0x0");

        dAgora = IDAgora(dAgoraAddr);
        isInitialized = true;
    }

    function activate() public {
        require(!isActive, "DAO already active");
        require(dAgora.bondStaked() >= ACTIVATION_THRESHOLD, "Threshold not met yet");
    ...
    {
        if (!isActive) {
            require(dAgora.bondStaked() >= ACTIVATION_THRESHOLD, "DAO not yet active");
            isActive = true;
        }

        require(
            dAgora.votingPowerAtTs(msg.sender, block.timestamp - 1) >= _getCreationThreshold(),
    ...
    function castVote(uint256 proposalId, bool support) public {
        ...
        uint256 votes = dAgora.votingPowerAtTs(msg.sender, _getSnapshotTimestamp(proposal));
    ...
    function abrogationProposal_castVote(uint256 proposalId, bool support) public {
        ...
        uint256 votes = dAgora.votingPowerAtTs(msg.sender, abrogationProposal.createTime - 1);
        require(votes > 0, "no voting power");
    ...
    function _canCancelProposal(uint256 proposalId) internal view returns (bool){
        ...
        if (msg.sender == proposal.proposer ||
            dAgora.votingPower(proposal.proposer) < _getCreationThreshold()
        ) {
            return true;
        }
    ...
    function _getCreationThreshold() internal view returns (uint256) {
        return dAgora.bondStaked().div(100);
    }
    ...
    function _getQuorum(Proposal storage proposal) internal view returns (uint256) {
        return dAgora.bondStakedAtTs(_getSnapshotTimestamp(proposal)).mul(proposal.parameters.minQuorum).div(100);
    }
    ...
    function _proposalAbrogated(uint256 proposalId) internal view returns (bool) {
        ...
        return cp.forVotes >= dAgora.bondStakedAtTs(cp.createTime - 1).div(2);
    }
}